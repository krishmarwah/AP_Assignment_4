...
public class Customer implements User {
    ...
    public void saveOrderHistoryToFile() {
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(fileName, true))) {  // Change true to false
            writer.write("Order History for Customer: " + name + "\n");
            if (orderHistory.isEmpty()) {
                writer.write("No orders yet.\n");
            } else {
                for (Order order : orderHistory) {
                    writer.write("Order ID: " + order.getOrderId() + "\n");
                    for (CartItem cartItem : order.getItems()) {
                        FoodItem foodItem = cartItem.getFoodItem();
                        writer.write("Food Item: " + foodItem.getName() + ", ");
                        writer.write("Quantity: " + cartItem.getQuantity() + ", ");
                        writer.write("Price: $" + foodItem.getPrice() * cartItem.getQuantity() + "\n");
                    }
                    writer.write("Total Price: $" + order.getTotalAmount() + "\n");
                    writer.write("Special Request: " + order.getSpecialRequest() + "\n");
                    writer.write("----------\n");
                }
            }
            writer.write("End of Order History for " + name + "\n\n");
        } catch (IOException e) {
